...
if torch is not None:
    ...
    class LSTMMarketModel(nn.Module):
        ...
        def forward_tensor(self, inputs: Tensor) -> tuple[Tensor, Tensor, Tensor]:
            ...
            if int(inputs.shape[1]) != self.sequence_length:
                raise ValueError(f"Expected sequence length {self.sequence_length}, received {int(inputs.shape[1])}.")
    ...
    class WalkForwardSplitter:
        ...
        def split(self, dataset_size: int) -> Iterator[tuple[list[int], list[int]]]:
            ...
            while start + self.train_size + self.validation_size <= dataset_size:
                ...
                validation_indices = list(range(validation_start, validation_start + self.validation_size))
                yield train_indices, validation_indices
                start += self.step_size


    class MarketModelTrainer:
        def __init__(self, model: LSTMMarketModel | None = None, config: MarketTrainingConfig | None = None) -> None:
        ...
        def build_feature_frame(
            ...
            raw_prices = self._load_market_data(symbols=symbols, start_date=start_date, end_date=end_date, price_source=price_source, price_frame=price_frame)
            ...
            for symbol in symbols:
                ...
                for as_of_date in symbol_dates:
                    history_records = gate.get(symbol=symbol, as_of_date=pd.Timestamp(as_of_date).date(), data_type="market")
            ...
            feature_frame = pd.DataFrame(rows).sort_values(["symbol", "as_of_date"]).reset_index(drop=True)
            feature_frame = self._attach_forward_returns(feature_frame)
            TimeGate().validate_no_lookahead(feature_frame.loc[:, ["symbol", "as_of_date", "available_at", *MARKET_FEATURE_COLUMNS]].copy())
        ...
        def build_sequences(self, feature_frame: pd.DataFrame) -> SequenceBundle:
            ...
            TimeGate().validate_no_lookahead(prepared.loc[:, ["symbol", "as_of_date", "available_at", *MARKET_FEATURE_COLUMNS]].copy())
            ...
            for symbol, group in prepared.groupby("symbol"):
                ...
                for idx in range(self.config.sequence_length - 1, len(group)):
                    ...
                    if pd.isna(target):
                        continue
                    window = group.loc[idx - self.config.sequence_length + 1 : idx, MARKET_FEATURE_COLUMNS]
            ...
            if metadata_frame.empty:
                return SequenceBundle(
                    sequences=np.empty((0, self.config.sequence_length, len(MARKET_FEATURE_COLUMNS)), dtype=np.float32),
                    targets=np.empty((0,), dtype=np.float32),
                    metadata=metadata_frame,
                )
            TimeGate().validate_no_lookahead(metadata_frame.copy())
            return SequenceBundle(sequences=np.asarray(sequences, dtype=np.float32), targets=np.asarray(targets, dtype=np.float32), metadata=metadata_frame)

        def fit(self, features: np.ndarray | Tensor, targets: np.ndarray | Tensor) -> MarketModelTrainer:
            ...
            self._train_model(self.model, self._make_loader(feature_tensor, target_tensor, shuffle=False))
            return self

        def predict_signal(self, sequence: np.ndarray | Tensor) -> MarketSignal:
            if self.model is None:
                raise ValueError("Model has not been initialized. Fit the trainer or provide a model first.")
        ...
        def evaluate(self, features: np.ndarray | Tensor, targets: np.ndarray | Tensor) -> tuple[float, float]:
            if self.model is None:
                raise ValueError("Model has not been initialized. Fit the trainer or provide a model first.")
            return self._evaluate_model(self.model, self._make_loader(self._to_feature_tensor(features), self._to_target_tensor(targets), shuffle=False))
        ...
        def fit_walk_forward(
            ...
            for fold_index, (train_indices, validation_indices) in enumerate(splitter.split(len(feature_tensor)), start=1):
                fold_model = LSTMMarketModel(int(feature_tensor.shape[-1]), hidden_size=self.config.hidden_size, dropout=self.config.dropout).to(self.device)
                self._reset_optimizers(fold_model)
                train_loss = self._train_model(fold_model, self._make_loader(feature_tensor[train_indices], target_tensor[train_indices], shuffle=False))
                validation_loss, directional_accuracy = self._evaluate_model(fold_model, self._make_loader(feature_tensor[validation_indices], target_tensor[validation_indices], shuffle=False))
        ...
        def walk_forward_validate(
            ...
            for anchor in range(self.config.train_months, len(periods) - self.config.test_months + 1, self.config.step_months):
                train_periods = periods[anchor - self.config.train_months : anchor]
                test_periods = periods[anchor : anchor + self.config.test_months]
                train_mask = bundle.metadata["as_of_date"].dt.to_period("M").isin(train_periods).to_numpy()
                test_mask = bundle.metadata["as_of_date"].dt.to_period("M").isin(test_periods).to_numpy()
                ...
                if bool((train_meta["available_at"] > pd.Timestamp(train_end)).any()):
                    violating = train_meta.loc[train_meta["available_at"] > pd.Timestamp(train_end)].iloc[0]
                    raise LookaheadError(f"training fold includes unavailable data: {violating['symbol']} {pd.Timestamp(violating['available_at']).isoformat()}")
                fold_model = LSTMMarketModel(int(bundle.sequences.shape[-1]), hidden_size=self.config.hidden_size, dropout=self.config.dropout).to(self.device)
                self._reset_optimizers(fold_model)
                train_loss = self._train_model(fold_model, self._make_loader(self._to_feature_tensor(bundle.sequences[train_mask]), self._to_target_tensor(bundle.targets[train_mask]), shuffle=False))
                validation_loss, directional_accuracy = self._evaluate_model(
                    fold_model,
                    self._make_loader(self._to_feature_tensor(bundle.sequences[test_mask]), self._to_target_tensor(bundle.targets[test_mask]), shuffle=False),
                )
                fold = FoldMetrics(
                    fold=len(folds) + 1,
                    train_loss=train_loss,
                    ...
                    max_train_available_at=train_meta["available_at"].max().to_pydatetime().replace(microsecond=0),
                    no_lookahead=True,
                )
                folds.append(fold)
        ...
        def _ensure_model(self, n_features: int) -> None:
            if self.model is None:
                self.model = LSTMMarketModel(n_features=n_features, hidden_size=self.config.hidden_size, dropout=self.config.dropout).to(self.device)
            elif self.model.n_features != n_features:
                raise ValueError(f"Model expects {self.model.n_features} features, received {n_features}.")

        def _reset_optimizers(self, model: LSTMMarketModel | None = None) -> None:
            target_model = model or self.model
            self.optimizer = AdamW(target_model.parameters(), lr=self.config.learning_rate, weight_decay=self.config.weight_decay)
        ...
        def _make_loader(self, features: Tensor, targets: Tensor, shuffle: bool) -> DataLoader:
            return DataLoader(MarketSequenceDataset(features, targets), batch_size=self.config.batch_size, shuffle=shuffle)
        ...
        def _evaluate_model(self, model: LSTMMarketModel, loader: DataLoader) -> tuple[float, float]:
            model.eval()
            total_loss = 0.0
            total_batches = 0
            correct = 0
        ...
        def _compute_loss(self, predicted_return: Tensor, direction_logit: Tensor, targets: Tensor) -> Tensor:
        ...
        def _to_feature_tensor(self, features: np.ndarray | Tensor) -> Tensor:
            tensor = features if isinstance(features, Tensor) else torch.as_tensor(features, dtype=torch.float32)
            if tensor.ndim != 3:
            ...
            if int(tensor.shape[1]) != self.config.sequence_length:
                raise ValueError(f"Expected sequence length {self.config.sequence_length}, received {int(tensor.shape[1])}.")
            return tensor.float()

        def _to_target_tensor(self, targets: np.ndarray | Tensor) -> Tensor:
            tensor = targets if isinstance(targets, Tensor) else torch.as_tensor(targets, dtype=torch.float32)
        ...
        def _load_market_data(
            ...
            if price_source in {"auto", "yfinance"}:
                try:
                    ...
                    if price_source == "yfinance":
                        raise
            return self._generate_synthetic_market_data(symbols=symbols, start_date=start_date, end_date=end_date)
        ...
        def _generate_synthetic_market_data(self, symbols: Sequence[str], start_date: date, end_date: date) -> pd.DataFrame:
            ...
            for symbol_index, symbol in enumerate(symbols):
                ...
                for day_index, current_day in enumerate(business_days):
                    latent_trend = (0.92 * latent_trend) + (0.05 * math.sin(day_index / 18.0)) + rng.normal(0.0, 0.05)
                    ...
                    low_price = min(open_price, close_price) * max(0.94, 1.0 - abs(rng.normal(0.0015, 0.001)))
                    volume = max(100_000.0, 1_000_000.0 * (1.0 + abs(latent_trend)) + rng.normal(0.0, 50_000.0))
                    rows.append(
                        {
                            "symbol": str(symbol),
        ...
        def _compute_feature_row(self, history: pd.DataFrame) -> dict[str, float]:
            ...
            bollinger_width = ((rolling_std * 4.0) / rolling_mean).replace([np.inf, -np.inf], np.nan).fillna(0.0)
            volume_zscore = ((volume - volume.rolling(window=20, min_periods=20).mean()) / volume.rolling(window=20, min_periods=20).std(ddof=0).replace(0.0, np.nan)).replace([np.inf, -np.inf], np.nan).fillna(0.0)
            realized_volatility = log_return.rolling(window=20, min_periods=20).std(ddof=0).fillna(0.0) * math.sqrt(20.0)
        ...
        def _attach_forward_returns(self, feature_frame: pd.DataFrame) -> pd.DataFrame:
            ...
            output["forward_return_5d"] = output.groupby("symbol")["close"].shift(-FORWARD_RETURN_HORIZON) / output["close"] - 1.0
            return output
    ...
    class LSTMMarketModel:  # pragma: no cover
        def __init__(self, *args, **kwargs) -> None:
            raise ImportError("torch is required to use LSTMMarketModel.")


    class MambaMarketModel(LSTMMarketModel):  # pragma: no cover
        pass


    class WalkForwardSplitter:  # pragma: no cover
        def __init__(self, *args, **kwargs) -> None:
            raise ImportError("torch is required to use WalkForwardSplitter.")


    class MarketModelTrainer:  # pragma: no cover
        def __init__(self, *args, **kwargs) -> None:
            raise ImportError("torch is required to use MarketModelTrainer.")
