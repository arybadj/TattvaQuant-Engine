...
class CompanyQualityModel:
    ...
    def fit(
        self, frame: pd.DataFrame, target_column: str = "forward_30d_return"
    ) -> CompanyQualityModel:
        ...
        self.prediction_bounds = (
            float(np.quantile(in_sample, 0.05)),
            float(np.quantile(in_sample, 0.95)),
        )
        self.fitted = True
        return self
    ...
    def predict_company_quality(self, features: dict[str, float]) -> CompanyQualitySignal:
        ...
        if self.fitted:
            ...
            model_score = (
                0.5 if math.isclose(low, high) else _clip01((reg_pred - low) / (high - low))
            )
    ...
    def predict_signal(
        self, features: dict[str, float], industry_context: str = ""
    ) -> FundamentalSignal:
        company = self.predict_company_quality(features)
        history = IndustryHistoryModel().predict(features)
        future = FutureIndustryModel(api_key=None).predict(
            features, industry_context=industry_context
        )
        return FundamentalSignal(
            long_term_strength=_clip01(
                (company.fundamental_score * 0.5)
                + (company.health_score * 0.3)
                + (history.stability_score * 0.2)
            ),
            growth_potential=_clip01(
                (company.valuation_score * 0.25)
                + (history.industry_cagr_5y * 0.25)
                + (future.future_growth_score * 0.5)
            ),
            risk_score=_clip01(
                (1.0 - company.health_score) * 0.5
                + future.disruption_risk * 0.35
                + (1.0 - future.macro_tailwind) * 0.15
            ),
    ...
    def build_training_frame(
        ...
        fundamentals = (
            fundamentals_frame.copy()
            if fundamentals_frame is not None
            else self.generate_synthetic_fundamentals(symbols, start_date, end_date)
        )
    ...
    def walk_forward_validate(
        ...
        sharpes: list[float] = []
        for anchor in range(
            self.train_months, len(periods) - self.test_months + 1, self.step_months
        ):
            ...
            if bool((train["available_at"] > pd.Timestamp(train_end)).any()):
                ...
                raise LookaheadError(
                    ...
                    f"{bad['symbol']} "
                    f"{pd.Timestamp(bad['available_at']).isoformat()}"
                )
            fold_model = CompanyQualityModel(random_state=self.random_state + anchor).fit(
            ...
            strategy = (
                np.where(probs >= 0.5, 1.0, -1.0)
                * np.clip(np.abs(probs - 0.5) * 2.0, 0.25, 1.0)
                * (1.0 + np.clip(reg, -0.2, 0.2))
                * realized
            )
            vol = float(strategy.std(ddof=0))
            sharpe = (
                0.0 if vol < 1e-12 else float((strategy.mean() / vol) * math.sqrt(len(strategy)))
            )
            ...
            folds.append(
                WalkForwardFoldResult(
                    ...
                    n_test=int(len(test)),
                    sharpe=sharpe,
                    max_train_available_at=train["available_at"]
                    .max()
                    .to_pydatetime()
                    .replace(microsecond=0),
                    no_lookahead=True,
        ...
        return WalkForwardBacktestResult(
            sharpe_series=sharpes, mean_sharpe=float(np.mean(sharpes)), folds=folds
        )

    def generate_synthetic_fundamentals(
        self, symbols: list[str], start_date: date, end_date: date, seed: int | None = None
    ) -> pd.DataFrame:
        ...
        for symbol_index, symbol in enumerate(symbols):
            ...
            for step, as_of_date in enumerate(months):
                ...
                revenue = float(
                    np.clip(
                        0.05 + (quality * 0.14) + (0.02 * seasonal) + rng.normal(0.0, 0.015),
                        -0.15,
                        0.45,
                    )
                )
                margin = float(
                    np.clip(0.12 + (quality * 0.15) + rng.normal(0.0, 0.012), 0.02, 0.45)
                )
                surprise = float(
                    np.clip(0.01 + (quality * 0.07) + rng.normal(0.0, 0.01), -0.15, 0.20)
                )
                components = {
                ...
                rows.append(
                    {
                        ...
                        "available_at": datetime.combine(
                            pd.Timestamp(as_of_date).date(), time(hour=8)
                        ),
                        **components,
                        ...
                        "sector_cagr_5y": float(
                            np.clip(0.10 + (0.015 * math.cos((step + 1) / 5.0)), 0.0, 0.30)
                        ),
                        ...
                        "currency_momentum_usdinr": float(
                            np.clip(rng.normal(0.01, 0.02), -0.10, 0.10)
                        ),
                    }
                )
    ...
    def _prepare_frame(
        self, frame: pd.DataFrame, require_targets: bool, target_column: str = "forward_30d_return"
    ) -> pd.DataFrame:
        data = frame.copy()
        ...
        if "piotroski_f_score" not in data.columns and all(
            column in data.columns for column in PIOTROSKI_COMPONENT_COLUMNS
        ):
        ...
        TimeGate().validate_no_lookahead(
            data.loc[:, ["symbol", "as_of_date", "available_at", *QUALITY_FEATURE_COLUMNS]].copy()
        )
        if require_targets:
            if target_column not in data.columns:
                raise ValueError(f"Missing required target column: {target_column}")
            data["direction_label"] = (
                (data[target_column].astype(float) > 0.0).astype(int)
                if "direction_label" not in data.columns
                else data["direction_label"].astype(int)
            )
    ...
    def _feature_row(self, features: dict[str, float]) -> dict[str, float]:
        ...
        score = float(
            row.get(
                "piotroski_f_score",
                sum(row.get(column, 0.0) for column in PIOTROSKI_COMPONENT_COLUMNS),
            )
        )
        ...
        if price_source in {"auto", "yfinance"}:
            try:
                ...
                rows = [
                    ...
                    for index, value in close[symbol].dropna().items()
                ]
                if rows:
                    return pd.DataFrame(rows)
            ...
            for day_index, day in enumerate(pd.bdate_range(start=start_date, end=end_date)):
                ...
                driver = (
                    ...
                    if latest.empty
                        ...
                        (latest["piotroski_f_score"].iloc[-1] / 9.0)
                        + latest["roe_3y_average"].iloc[-1]
                        + latest["free_cash_flow_yield"].iloc[-1]
                    )
                )
                price *= max(
                    0.80,
                    ...
                    + 0.0002
                    + (driver * 0.0007)
                    + (0.0004 * math.sin(day_index / 21.0))
    ...
    def _attach_targets(self, fundamentals: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol, price_group in prices.groupby("symbol"):
            ...
            aligned = calendar.reindex(
                pd.date_range(calendar.index.min(), calendar.index.max(), freq="D")
            ).ffill()
            for record in fundamentals.loc[fundamentals["symbol"] == symbol].to_dict(
                orient="records"
            ):
                ...
                rows.append(
                    {
                        **record,
                        "forward_30d_return": float(forward_return),
                        "direction_label": int(forward_return > 0.0),
                    }
                )
...
class FutureIndustryModel:
    ...
    def predict(self, features: dict[str, float], industry_context: str) -> FutureIndustrySignal:
        ...
        prompt = (
            ...
            "Return JSON with future_growth_score, disruption_risk, "
            "macro_tailwind as floats in [0,1]."
        )
        payload = {
            "model": self.model,
            "input": [
                    ...
                    "content": [
                        {
                            "type": "input_text",
                            "text": json.dumps(
                                {"macro_features": features, "industry_context": industry_context}
                            ),
                        }
                    ],
                },
    ...
    def _heuristic_predict(
        ...
        future_growth_score = max(
            min(0.5 + tam_trend * 0.4 + patent_trend * 0.3 + rate_regime * 0.1, 1.0), 0.0
        )
...
class FundamentalModelEnsemble:
    ...
    def predict(
        ...
        long_term_strength = _clip01(
            (company.fundamental_score * 0.45)
            + (company.health_score * 0.25)
            + (history.stability_score * 0.30)
        )
        growth_potential = _clip01(
            (history.industry_cagr_5y * 0.35)
            + (future.future_growth_score * 0.45)
            + (future.macro_tailwind * 0.20)
        )
        risk_score = _clip01(
            (1.0 - company.health_score) * 0.35
            + future.disruption_risk * 0.40
            + abs(feature_row.get("debt_to_equity_delta", 0.0)) * 0.25
        )
        return FundamentalSignal(
